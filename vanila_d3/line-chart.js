...
const lb_filtered_condition = ["Uric Acid(\uac80\uc0ac24\uc2dc\uac04\uac00\ub2a5)", 'GPT (ALT)(검사24시간가능)', 'GOT (AST)(검사24시간가능)',
    'Creatinine(검사24시간가능)', 'GOT (AST)(\uac80\uc0ac24\uc2dc\uac04\uac00\ub2a5)',
    "Creatinine(\uac80\uc0ac24\uc2dc\uac04\uac00\ub2a5)", 'GPT (ALT)(\uac80\uc0ac24\uc2dc\uac04\uac00\ub2a5)']
...
let makingcheckBox = (label, fieldName) => {
    $("#div_chk").append(fieldName[0].toUpperCase()+fieldName.substring(1)+"<br>")
...
let drawLineGraph = (data, dataLabel, dataMargin, svgId) => {
    ...
    //     .style("opacity", 0);
    const svg = d3.select(chart_div_id)
        .append('svg')
        .style('width', width)
        .style('height', height)
        .attr("id", svgId);
...
const drawingDot = (data, dataLabel, dataMargin, svgId) => {
    ...
    const svg = d3.select(chart_div_id).select("#"+svgId)
        .append('svg')
        .style('width', width)
        .style('height', height)
        .attr("id", svgId+"dot");
...
let makingDot = (dataList, dataLabel, dataMargin, title) => {
    ...
    for (const data of dataList) {
        if (!!data) {
            drawingDot(data, dataLabel[cur_number], dataMargin, title+cur_number.toString());
        }
        cur_number += 1;
    }
}
...
let makingLine = (dataList, dataLabel, dataMargin, title) => {
    ...
    for (const data of dataList) {
        if (!!data) {
            drawLineGraph(data, dataLabel[cur_number], dataMargin, title+cur_number.toString());
        }
        cur_number += 1
    }
}
    ...
    for (const [date, values] of Object.entries(events)) {
        ...
        values.med.forEach((data) => {
            ...
            if (originMdLabel.includes(med_name)) {
                ...
                originMdList[med_index].push({date: new Date(date), value: prescription})
            } else {
